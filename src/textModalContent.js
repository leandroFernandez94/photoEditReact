...
class TextModalContent extends React.Component {
    ...
    limitarTexto = (texto) => {
        if (this.textP.clientHeight + 1 > 354 || this.textP.clientWidth + 1 > 520) {
            this.text.value = texto.slice(0,texto.length -1)
        }
    }
    limitarFontSize = (fontSize) => {
        if (this.textP.clientHeight + 1 > 354 || this.textP.clientWidth + 1 > 520) {
            this.textFontSize.value =  fontSize /2
            this.props.texto.size = this.textFontSize.value /2
        }
    }
        ...
        <div id="modal-import-container">
                ...
                <form style={{padding: '30px'}} className="row">
                    <div className={'col-8'}>
                        <div className="form-group">
                            ...
                            <input type="text" className="form-control" ref={c => this.text = c} onChange={evt => {
                                this.limitarTexto(evt.target.value);
                                this.props.saveText(evt.target.value, 'texto')
                            }} placeholder="Mi texto"/>
                        </div>
                        <div className="form-group">
                            <label>Tamaño de letra:</label>
                            <input type="number" ref={c => this.textFontSize = c} className={'form-control'}
                                   onChange={evt => {
                                evt.target.value = Math.min(evt.target.value, 40);
                                this.props.saveText(evt.target.value + 'px', 'size')
                            }} placeholder="12"/>
                        </div>
                        <div className="form-group">
                            <label htmlFor="exampleSelect1">Tipografia:</label>
                            <select className="form-control" id="exampleSelect1"
                                    onChange={evt => this.props.saveText(evt.target.value, 'font')}>
                    ...
                    <div className="form-group col-12">
                        ...
                        <div className="row" style={{display: 'contents'}}>
                            <div style={{
                                ...
                                <p className={this.props.texto.font} style={{
                                    fontSize: this.props.texto.size,
                                    color: this.props.texto.color,
                                    display: 'inline-block'
                                }} ref={c => this.textP = c}

                                > {this.props.texto.texto}</p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="row horizontal-center">
                <button className="btn btn-outline-info btn-sm btn-margin"
                        onClick={() => this.props.agregarTexto(this.textP.clientHeight + 1, this.textP.clientWidth + 1, this.textP)}>Generar
                </button>
            </div>
